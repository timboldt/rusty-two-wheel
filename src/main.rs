...
fn main() -> ! {
    ...
    // * System clock at 48MHz (maximum value of 72MHz, but causes problems with PS2 controller).
    // * APB1 peripheral clock restricted to 24MHz (its maximum value is 36MHz).
    let clocks = rcc
        .cfgr
        .use_hse(8.mhz())
        .sysclk(48.mhz())
    ...
    disp.init().unwrap();
    let _ = disp.clear();
    let _ = disp.write_str("   Initialized  ");
    ...
    let _ = disp.write_str("Begin main loop ");

    loop {
        let start = mono.now();
        let _ = disp.write_str("0123456789012345");
        let elapsed = start.elapsed();
        let _ = writeln!(disp, "elapsed: {}", elapsed);
